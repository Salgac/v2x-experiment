...
# commercial NSA network (Orange SK by default) using QMI as the primary
# control path, and verify real connectivity before anything else (the
# AT-layer logger, main.py, etc.) is started against this modem.
#
# Everything this session discovered is baked in here:
#   - QMI "mode preference" (lte|5gnr), not just the AT-layer AT+CNMP,
#     is the setting that actually gates SA vs NSA acquisition.
#   - PLMN must be pinned MANUALLY (not "automatic") -- this SIM only has
#     a roaming agreement onto Orange SK's real PLMN (231-01); "automatic"
#     selection was observed drifting onto a neighboring, forbidden PLMN
#     (231-02) instead.
#   - APN is passed directly on the QMI data call (WDS), not via AT+CGDCONT
#     -- keeps the whole flow QMI-native rather than mixing AT and QMI.
#   - is_connected() below does an EXACT status match, not a substring
#     match -- the earlier AT-based ue_connect.sh had a real bug here
#     (grep -qi "connected" also matches "disconnected").
#
...
PLMN="${PLMN:-23101}"                       # Orange SK, MCC 231 + MNC 01 -- pin manually,
                                             # "automatic" was observed drifting onto 231-02
APN="${APN:-internet.static}"               # this SIM's correct APN (not the private-core "internet")
...
# ===========================================================================
log "Setting QMI mode preference='$MODE_PREF', PLMN=$PLMN (manual)..."
qmicli -p -d "$QMI_DEV" --nas-set-system-selection-preference="${MODE_PREF}",manual="${PLMN}" \
